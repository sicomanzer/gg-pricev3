...
const Index = () => {
  ...
  const { saveScanResults, updateOpenPositions, records, isLoading: isHistoryLoading } = useBacktest();
  ...
  const handleScan = useCallback(async (params: ScanParams) => {
    ...
    if (quotes.length > 0) {
      ...
      // Update Backtest Status (Check TP/SL)
      await updateOpenPositions(quotes);
  ...
  }, [fetchStockData, checkAlerts, records, updateOpenPositions, saveScanResults]);
